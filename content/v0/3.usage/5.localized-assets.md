...
You can create the asset reference using the [`i18n_create_ref_asset()`](/v0/api-reference/functions/i18n-create-ref-asset) function. The function is really similar to the [`i18n_create_ref_message()`](/v0/api-reference/functions/i18n-create-ref-message) function. Here's some difference between them:

1. The [`i18n_create_ref_asset()`](/v0/api-reference/functions/i18n-create-ref-asset) function doesn't have the `key` parameter. You don't need to provide the key, because the key is the asset itself.
2. The [`i18n_create_ref_asset()`](/v0/api-reference/functions/i18n-create-ref-asset) function has the `locale_asset` parameter. You need to provide the asset for each locale in the struct. The struct key is the locale code, and the value is the asset ID.
...
> Creates a reference to the localized asset. The reference will automatically update itself when the locale is changed by calling [`i18n_set_locale()`](/v0/api-reference/functions/i18n-set-locale) function.
...
> `Asset` (in the **current locale** of the [`I18n`](/v0/api-reference/functions/i18n-create) struct)
...
You can see the detailed explanation in the [Variable Name Handling](/v0/usage/messages#variable-name-handling) section.
::
