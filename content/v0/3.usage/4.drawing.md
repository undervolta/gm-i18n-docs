...
Drawing is an important part of any game. In GM-I18n, you can draw the translated text using the [`i18n_draw_message()`](/v0/api-reference/functions/i18n-draw-message) function. It's a wrapper for the `draw_text_*` functions with convenient additional features, so you can use it just like the `draw_text_*` functions.
...
> [`I18nDrawings`](/v0/api-reference/constructors#i18ndrawings)
...
1. Using the [`i18n_draw_message()`](/v0/api-reference/functions/i18n-draw-message) function. You'll get some benefit from using this function.
2. Using the [`i18n_use_drawing()`](/v0/api-reference/functions/i18n-use-drawing) function to change the drawing configuration, and then use the `draw_text_*` or [`i18n_draw_message()`](/v0/api-reference/functions/i18n-draw-message) functions.

**What's make the [`i18n_draw_message()`](/v0/api-reference/functions/i18n-draw-message) function different from the `draw_text_*` functions?**
...
2. Built-in support for the drawing presets. You can easily change the `font`, `color`, `alignment`, etc. by just passing the preset name to the function. See the [Drawing Presets](/v0/usage/drawing#drawing-presets) section for more information.
...
::alert{type="info"}
Currently, the [`i18n_draw_message()`](/v0/api-reference/functions/i18n-draw-message) function is the only way to directly draw the translated text in GM-I18n.
::

::alert{type="warning"}
The [`i18n_draw_message()`](/v0/api-reference/functions/i18n-draw-message) function is not suitable for complex text drawing. For example, you can't use it to draw a text with multiple font size, color, or alignment in a single line. <br> <br>

Don't pass the same `preset_name` to the [`i18n_draw_message()`](/v0/api-reference/functions/i18n-draw-message) function in sequential calls. It will call a redundant [`i18n_use_drawing()`](/v0/api-reference/functions/i18n-use-drawing) function, which is not efficient. <br> <br>

You can't use named data interpolation with the [`i18n_draw_message()`](/v0/api-reference/functions/i18n-draw-message) function. You need to use the [`i18n_get_messages()`](/v0/api-reference/functions/i18n-get-messages) or [`i18n_get_ref_message()`](/v0/api-reference/functions/i18n-get-ref-message) function to get the message first, and then pass the message to the [`i18n_draw_message()`](/v0/api-reference/functions/i18n-draw-message) or `draw_text_*` function.
::
