...
::alert{type="info"}
As you can see, the chunks will be loaded in the order they are defined in the `I18nLocaleInit` array, and the `time` option will control how long it takes to load each chunk. <br> <br>
...
i18n_draw_message(100, 150, "@:button");                                // "Click Me" in en.json, "Klik Saya" in id.json, "クリックしてください" in ja.json
