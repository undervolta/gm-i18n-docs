...
1. Get a message using [`i18n_get_messages()`](/v0/api-reference/functions/i18n-get-messages), or create a message reference using [`i18n_create_ref_message()`](/v0/api-reference/functions/i18n-create-ref-message).
...
You can register dictionaries using the [`i18n_add_dictionaries()`](/v0/api-reference/functions/i18n-add-dictionaries) function. It's similar to [`i18n_add_messages()`](/v0/api-reference/functions/i18n-add-messages), but it's only accepts data in array format, where each item is a pair of key and value, like `["key", "value"]`.
...
::alert{type="info"}
You normally register dictionaries for **other locales** than the default locale (in this case, `en`), so you can use the [`i18n_add_dictionaries()`](/v0/api-reference/functions/i18n-add-dictionaries) function to register dictionaries for `id` and `ja` locales as shown in the example above. <br> <br>
...
::alert{type="info"}
You can use the [`i18n_get_messages()`](/v0/api-reference/functions/i18n-get-messages) or [`i18n_create_ref_message()`](/v0/api-reference/functions/i18n-create-ref-message) function to get a message or create a message reference with the `data` that contains the dictionary value. The system will automatically use the dictionary value if it exists, or the data value as is if it doesn't. <br> <br>
...
::alert{type="warning"}
You **can't** use dictionaries in an [indexed data interpolation](/v0/usage/interpolation#indexed-data), like `{0}`, `{1}`, etc. Dictionaries only work with [named data interpolation](/v0/usage/interpolation#named-data), like `{item}` or `{name}`. <br> <br>
