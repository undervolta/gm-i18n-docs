...
::alert{type="info"}
You can use the `i18n_get_messages()` or `i18n_create_ref_message()` function to get a message or create a message reference with the `data` that contains the dictionary value. The system will automatically use the dictionary value if it exists, or the data value as is if it doesn't. <br> <br>
...
::alert{type="warning"}
You **can't** use dictionaries in an [indexed data interpolation](/v0/usage/interpolation#indexed-data), like `{0}`, `{1}`, etc. Dictionaries only work with named data interpolation, like `{item}` or `${item}`. <br> <br>
